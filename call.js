...
async function loadJSON() {
  const response = await fetch('./priming.json');
  priming  = await response.json();
  console.log(priming);
}
...
const loadDialog = (n) => {
  ...
  const prompt01 = priming.mats[n].instruction + priming.mats[n].content;
  dialog = prompt01;
  ...
  //console.log("make call",prompt)
    ...
    console.log('making your request...');
      ...
      const response = await openai.createCompletionFromModel({
        ...
        let rawData = response.data.choices[0].text.replace('###', '').replace('A:', '');
 
        
        let index = rawData.indexOf('B:');
        console.log(`Bindex: ${index}`);

        let testD = "The scene changes, like a flash or something, then everything is different.\nB:I'm in a van, a big van, something like my family had.";
        let index1 = testD.indexOf('B:');
        console.log(`Index-test: ${index1}`);
        ...
        let displayText = `benet83: ${newRawData.replace('###', '').replace('A:', '').replace('B:', '').replace('\n', '. ')}`;
